...
def getknowns(email):
    ...
    for i in users:
        ...
        if(i['n.ip']!=None and i['n.ip']!="Bad Gateway"):
...
def checkwebques(query,stype,words,pos):
    check=False
    if("bot" not in words and "name" not in words and "bot" not in words and "botmaster" not in words and "my" not in words and "your" not in words and "bot master" not in query and "master" not in query):
        if stype == "Question" and pos[-2][1] in nouns and len(
                words) < 7 and "Who is" in query or "What is " in query :
            check = True
...
def get_bot_response():
    ...
    if response and response!="Recursion limit exceeded." and response!="none" and response!="None" and response!=" " and response!="unknown" and response!="":
        response2=response
        chat = f"\nbot : " + response2
        ...
        if(len(known)!=0):
            ...
            response2 = f"Do you know {known[0]} ?"
            learn()
            ...
            if(mood!="neutral"):
                response2=f"You seem {mood} today."
                q10=f"I am very {mood} today."
                bot.respond(q10,name)
                learn()

            else:
                response2 = "Apologies, I'm unable to provide a response right now. Please try again later or ask another question. Thank you!"
                learn()
